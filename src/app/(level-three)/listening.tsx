...
const DATA: Message[] = [
  {
    ...
    englishAudioResources: {
      gender: "F",
      source:
        "https://res.cloudinary.com/vanilajs/video/upload/v1726104201/app/flezxzvsmd1do6tdaerj.mp3",
      // source:
      //   "https://res.cloudinary.com/vanilajs/video/upload/v1725950488/app/mro5bjw0dhkkxodgvjkv.mp3",
    },
    nativeAudioResources: {
      gender: "F",
      lang: "fr",
...
type MessageRowProps = {
  ...
  index: number;
  activeRow: ActiveRow;
  playEnglishAudio: (source: string) => Promise<void>;
  playNativeAudio: (source: string) => Promise<void>;
};
...
function Listening() {
    ...
    async (currentActiveNumber: number) => {
      try {
        ...
        const { sound: soundResponse } = await Audio.Sound.createAsync(
          {
            uri: currentRowAudio.englishAudioResources.source,
          },
          {},
  ...
  const playEnglishAudio = async (audioSource: string) => {
    try {
      await playAudio(audioSource as unknown as AVPlaybackSource);
    } catch (error) {
  ...
  const playNativeAudio = async (audioSource: string) => {
    try {
      await playAudio(audioSource as unknown as AVPlaybackSource);
    } catch (error) {
